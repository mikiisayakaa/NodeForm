...
void Nodest::imageDisplay(const QImage &srcImage, QImage &rstImage)
{
...
void Nodest::loadImage(const QString &url, QImage &image)
{
    if(url == ""){
        return;
    }
...
void Nodest::inverseImage(const QImage &srcImage, QImage &rstImage)
{
    rstImage = srcImage;
...
void Nodest::brightness(const QImage &srcImage, const int &val, QImage &rstImage)
{
    rstImage = srcImage;
    for (int i = 0; i < rstImage.height(); i++){
        QRgb* line = (QRgb*)rstImage.scanLine(i);
...
void Nodest::grayScale(const QImage &srcImage, QImage &rstImage)
{
    rstImage = srcImage;
